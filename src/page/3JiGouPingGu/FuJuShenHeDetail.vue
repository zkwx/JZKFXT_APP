      ...
      <div v-transfer-dom>
        <x-dialog v-model="showScrollBox" hide-on-blur >
          ...
          <div style="padding: 0 20px 20px 20px;">
            <check-icon style="margin-right:2em;margin-bottom:1em;":value.sync="Agree">阅读并同意</check-icon>
            <x-button :disabled="Agree===false" @click.native="showQuestion=true;showScrollBox=false" type="primary">开始答题</x-button>
          </div>
        </x-dialog>
      </div>
    </group>
  ...
  methods: {
    ...
    submit() {
      ...
      if(!this.disabledInfo.ID){
        this.$http.post('DisabledInfoes', this.disabledInfo).then(r => {
          ...
          _this.$api.GetExam(this.targetExamName).then(r => { 
            _this.ExamID=r.ID
            _this.targetExamName=r.Name
            _this.Questions=r.Questions
            this.initQuestion()
          })
    ...
    SubmitAnswers(){
      ...
      this.$http.post('Answers/SaveAnswers', Answers).then(r => {
          $utils.Alert('适配成功，适配结果为：',AssistiveDevices)
